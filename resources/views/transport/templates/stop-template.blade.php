...
>
    <p class="text-xl font-semibold"><%= station && station.name !== undefined ? station.name : stops.name %></p>

        <div class="flex-1">
            <form method="POST" action="/" id="trips" class="bg-white p-3 rounded-lg shadow-md">
                        ...
                        <div class="mt-2 space-y-2">
                            <% transportOptions.forEach(function(option) { %>
                                <div class="flex items-center">
                                    ...
                                    <label for="<%= option %>_arrival" class="ml-2 block text-sm text-gray-700 capitalize"><%= option.replace('_', ' ') %></label>
                                </div>
                            <% }); %>
                        </div>
                    </fieldset>
