                            ...
                            @if (isset($this->stop))
                                ...
                                <div class="mb-3">
                                    ...
                                    <div class="grid grid-cols-2 gap-2">
                                        <input type="date" name="date" wire:model.change="date" value="{{ $date }}" class="mt-1 block w-full rounded-md border-gray-300 shadow-sm focus:border-indigo-500 focus:ring-indigo-500" />
                                        <div class="grid grid-cols-2 gap-2">
                                            <input type="number" name="hour" value="{{ $hour }}" wire:model.change="hour" class="mt-1 block w-full rounded-md border-gray-300 shadow-sm focus:border-indigo-500 focus:ring-indigo-500" />
                                            <input type="number" name="minute" value="{{ $minute }}" wire:model.change="minute" class="mt-1 block w-full rounded-md border-gray-300 shadow-sm focus:border-indigo-500 focus:ring-indigo-500" />
                                        </div>
                                    </div>
                                </div>
                                <div class="mb-3">
                                    <label class="block text-sm font-medium text-gray-700">Duration (minutes)</label>
                                    <input type="number" name="duration" wire:model.change="duration" value="{{ $duration }}" min="1" class="mt-1 block w-full rounded-md border-gray-300 shadow-sm focus:border-indigo-500 focus:ring-indigo-500" />
                                </div>
                                <p>{{json_encode($options)}}</p>

                                @if ($trips)
                                    @foreach ($trips as $dep)
                                        <div class=" grid grid-cols-2">
                                            <p class="text-lg">{{$dep->line->name ?? ''}}</p>
                                            <p>{{$dep->direction}}</p>
                                        </div>
                                        <div class="mb-3 grid grid-cols-2">
                                            <div>
                                                <p class="time-tracker" data-timestamp="{{$dep->when}}"></p>
                                                @if (!empty($dep->when) && $dep->when != $dep->plannedWhen)
                                                    <p class="time-tracker @if (!empty($dep->when)) text-red-500 line-through @endif" data-timestamp="{{$dep->plannedWhen}}"></p>
                                                @endif
                                            </div>
                                            <div class="">
                                                @if ($dep->remarks)
                                                    @foreach ($dep->remarks as $remark)
                                                        <p class="text-sm " data-text="{{$remark->text}}">{{ Str::words($remark->text, 10, '...') }}</p>
                                                    @endforeach
                                                @endif
                                            </div>
                                        </div>
