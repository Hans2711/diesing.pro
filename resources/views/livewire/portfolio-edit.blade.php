            ...
            @if ($activePortfolio->media->count() > 0)
                <div class="my-3 flex flex-wrap gap-3">
                    @foreach ($activePortfolio->media as $photo)
                        <div class="">
                            <img class="w-24 h-24 object-cover rounded border" src="{{ Storage::url($photo->path) }}" alt="{{ $photo->name ?? '' }}" title="{{ $photo->name ?? '' }}">
                ...
                @if ($media)
                    ...
                    @foreach ($media as $photo)
                        @if (in_array($photo->getMimeType(), ['image/jpeg', 'image/png', 'image/gif']))
                            <img class="w-24 h-24 object-cover rounded border" src="{{ $photo->temporaryUrl() }}" alt="{{ __('alt.view') }}" title="{{ __('alt.view') }}">
                        @endif
                    @endforeach
                    </div>
                @endif
