...
document.addEventListener("DOMContentLoaded", function () {
  ...
  var redirectsWrapper = document.querySelector("#redirects-wrapper");
    ...
    editButtons.forEach(function (button) {
      button.addEventListener("click", function () {
        ...
        for (let i = 0; i < redirectModalCodeInput.options.length; i++) {
          if (
            redirectModalCodeInput.options[i].value ==
  ...
  addRedirectButton.addEventListener("click", function () {
    redirectModalIdInput.value = null;
    redirectModalNameInput.value = null;
    redirectModalTargetInput.value = null;
    redirectModalUrlInput.value = null;

    redirectModal.classList.remove("hidden");
  });

  function updateRedirectsView() {
    fetch("/privater-bereich/weiterleitungen/get-list")
      .then(function (response) {
        return response.text();
      })
      .then(function (html) {
        redirectsWrapper.innerHTML = html;
        resetDeleteButtons();
        resetEditButtons();
      });
  }
  ...
  redirectModalClose.addEventListener("click", function () {
    updateRedirect(
      redirectModalIdInput.value,
      redirectModalNameInput.value,
      redirectModalTargetInput.value,
      redirectModalCodeInput.value,
  ...
  function deleteRedirectButton(button) {
    if (confirm("Sicher, dass du die Weiterleitung löschen willst?")) {
